...
pub fn array_routes(auth_state: AuthState) -> Router {
    Router::new()
        ...
        .route("/api/v1/arrays/:name/scrub", post(handlers::scrub_array)
            .route_layer(middleware::from_fn(auth::require_role(UserRole::Operator))))
        .route("/api/v1/health/disks", post(handlers::analyze_disk_health)
            .route_layer(middleware::from_fn(auth::require_role(UserRole::Operator))))
        .route("/api/v1/migrations/start", post(handlers::start_migration)
            .route_layer(middleware::from_fn(auth::require_role(UserRole::Admin))))
        .route("/api/v1/migrations/pause", post(handlers::pause_migration)
            .route_layer(middleware::from_fn(auth::require_role(UserRole::Admin))))
        .route("/api/v1/migrations/resume", post(handlers::resume_migration)
            .route_layer(middleware::from_fn(auth::require_role(UserRole::Admin))))
        .route("/api/v1/cluster/nodes", get(handlers::list_cluster_nodes))
        .route("/api/v1/cluster/nodes", post(handlers::join_cluster)
            .route_layer(middleware::from_fn(auth::require_role(UserRole::Admin))))
        .route("/api/v1/cluster/sync", post(handlers::sync_cluster_metadata)
            .route_layer(middleware::from_fn(auth::require_role(UserRole::Admin))))
